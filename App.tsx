...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen bg-emerald-50/30 pb-20 transition-all duration-300 ${isUrdu ? 'rtl font-amiri' : 'ltr'}`} dir={isUrdu ? 'rtl' : 'ltr'}>
      {/* Header */}
      <header className="bg-emerald-900 text-white py-6 px-4 shadow-lg mb-8 sticky top-0 z-50">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 bg-white/5 p-1 rounded-full border border-white/10">
            ...
            <button 
                onClick={() => setLang('ur')}
                className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${lang === 'ur' ? 'bg-emerald-600 text-white shadow-md' : 'text-emerald-200 hover:text-white'}`}
            >
                اردو
            </button>
          </div>
